...
Este repositorio e um no independente gerenciado pelo hub `togs-backoffice` com o id `jogos-de-hoje`. Quando um caminho local for necessario, resolva-o pelo control plane ou pela variavel `TOGS_BACKOFFICE_PATH`; nao publique paths de usuario neste repositorio.
